                        ...
                        ()=>{
                            if (app.get().game.choices[0].name == 'Skip'){
                                ...
                                limbo.push(
                                    ...
                                    ...hand.filter(
                                            (el)=>{card.identifier == el.identifier}
                                        )
                                    }
                                );
                                 hand = hand.filter(
                    ...
                    'Colonizing your Planet':
                        ()=>{        
                            if (app.get().game.acting_player.activeaction != 'colonize' || app.get().game.choices[0].name != 'colonize'){
                                ...
                                app.colonize(app.get().game.subchoices[0], app.get().game.acting_player.limbo , app.get().game.acting_player.limbo.filter((el)=>{ el.type == 'colonize'})[0]);
                                app.phasefinishfunction();
                            }
                        }
                },
                ...
                },
                {
                    ...
                    ()=>{        
                        if (app.get().game.acting_player.activeaction != 'politics' ){
                            ...
                            app.politics(app.get().game.acting_player.limbo.filter((el)=>{ el.type == 'politics'})[0], app.get().game.choices[0], app.get().game.acting_player);
                            app.phasefinishfunction();
                        }
                    }
                },
                    ...
                    'Colonizing your Planet':
                        ()=>{        
                            if (app.get().game.acting_player.activeaction != 'improved_colonize' || app.get().game.choices[0].name != 'colonize'){
                                ...
                                app.colonize(app.get().game.subchoices[0], app.get().game.acting_player.limbo , app.get().game.acting_player.limbo.filter((el)=>{ el.type == 'improved_colonize'})[0]);
                                app.phasefinishfunction();
                            }
                        }
                },
                    ...
                    'Terraforming your Planet':
                        ()=>{        
                            if (app.get().game.acting_player.activeaction != 'colonize' || app.get().game.choices[0].name != 'colonize'){
                                ...
                                app.colonize(app.get().game.choices[0], app.get().game.acting_player.limbo , app.get().game.acting_player.limbo.filter((el)=>{ el.type == 'terraforming'})[0]);
                        ...
                        if (cards[0].name=='Skip'){
                            ...
                            for (let i in cards){
                                ...
                                limbo.push(
                                    {'final_destination_label':'discard', 
                                    ...hand.filter(
                                            (el)=>{cards[i].identifier == el.identifier}
                                        )
                                    }
                                );
                                 hand = hand.filter(
                        ...
                        ()=>{        
                            if (app.get().game.acting_player.activerole != 'colonize' || app.get().game.choices[0].name != 'colonize'){
                                ...
                                for (let i = 0; i < app.get().game.acting_player.boostingicons.colonize; i++ ){
                                    ...
                                    app.colonize(planet, app.get().game.acting_player.limbo , app.get().game.acting_player.limbo.filter((el)=>{ el.type == 'colonize'})[0]);
                                }
                                app.phasefinishfunction();
                            }
                        }
            ...
            [
                {
                    ...
                    ()=>{  
                        app.offer(
                            false /*option to skip | sets game.displayinfo.showoptiontoskip=boolean */,
                            false /*allows multiple choices | sets game.displayinfo.allowformultipleselections=boolean */, 
                            ['options', [{name:'dissent'}, {name:app.get().game.leadingplayer.activerole.type}]] /* available cards to choose from | game.displayinfo.selectionzone={'hand|discard|options|planets|research|rolecards'}, sets choices=array if specified*/, 
                            'choices' /* label for where the choice is stored | set with game[label]=*/,
                            app.phasefinishfunction /*callback that handles the choice or finishes the phase*/, 
                        );
                    }
                ...
                //         -> colonize
                {
                    'Choose between Settling or Colonizing a Planet':
                        ()=>{   
                            if (app.get().game.acting_player.activerole!='colonize'){
                               app. phasefinishfunction();
                            } else if ( game.acting_player.permanents.filter( (el)=>{el.type=='bureaucracy'} ).length == 0){
                                let game = app.get().game;
                    ...
                    ()=>{ 
                        if(choices[0].name!="Skip"){
                            ...
                            for (let i in game.choices){
                                app.discard(game.acting_player.hand, game.acting_player.discard, game.choices[i].identifier);
                            }
                            app.send({'game':game});
                            app.phasefinishfunction();
                        }
